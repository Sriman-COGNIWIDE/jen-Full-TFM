module "vm_nginx" {
  ...
  remote_exec_cmds = ["sudo yum update",
    ...
    "sudo systemctl enable nginx > /dev/null 2>&1", "sudo echo '${sensitive(module.vm_connect.tls_key)}' > vm_connect_key.pem",
...
module "vm_app_1" {
  ...
  add_custom_policies    = true
  custom_policy_files    = [file("${path.module}/s3_access_policy.json")]
