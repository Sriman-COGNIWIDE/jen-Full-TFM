...
resource "aws_iam_policy" "custom_policy" {
  count       = var.create_iam_role && var.add_custom_policies ? length(var.custom_policy_files) : 0
  name        = "${var.instance_name}-policy-${count.index}"
  description = "custom policy added"
  policy      = var.custom_policy_files[count.index]

  depends_on = [aws_iam_role.ec2_assume_role]
}
...
resource "aws_iam_role_policy_attachment" "secrets_access_attachment" {
  count      = var.create_iam_role && var.add_policy_arns ? length(var.policy_arns) : 0
...
resource "aws_iam_instance_profile" "ec2_profile" {
  ...
  depends_on = [aws_iam_role_policy_attachment.custom_policy_attachment, aws_iam_role_policy_attachment.secrets_access_attachment]
}
